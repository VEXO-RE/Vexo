...
Get-ChildItem -Path "$BASE\public\images\Desarrollos" -Recurse -Include "*.webp","*.jpg","*.png","*.jpeg" -File -ErrorAction SilentlyContinue | ForEach-Object {
    $imgTotal++
...
# Verificar que data.js referencia .webp (no .jpg)
if ($dataJs -match '"images/Desarrollos/.*\.webp"') { OK "data.js — fotos referencian .webp correctamente" }
else { WARN "data.js — revisa rutas de imágenes (deben ser images/Desarrollos/*.webp)" }

# Nota sobre visibilidad de imágenes
WARN "Asegúrate de no usar rutas VEXO_WEB en data.js. Debe cargar imágenes internas desde images/Desarrollos/"
...
$excluded = @(".env", "*.csv", "*.xlsx", "Mapa-vexo/", "Proyectos/", "*.log", "*.py", "Sincronizar_Vexo.ps1")
