...
# ================================================================
HDR "2. Verificacion public/data.js y public/data.generated.js"

$dataPath = "$BASE\public\data.js"
$generatedPath = "$BASE\public\data.generated.js"
if (-not (Test-Path $dataPath)) {
    ERR "public/data.js no existe — ejecutar actualizar_contenido.ps1 primero"
} else {
    ...
    if ($encOK) {
        $msg = "data.js encoding limpio ($szKB KB)"
        OK $msg
    }

    # b) Acentos existen
    if ($data -match 'M[ée]rida') { OK "Acento Merida: OK" } else { ERR "No se encontro Merida con acento" }
    if ($data -match 'Yucat[aá]n') { OK "Acento Yucatan: OK" } else { WARN "No se encontro Yucatan con acento" }
    if ($data -match 'Temoz[oó]n') { OK "Acento Temozon: OK" } else { WARN "No se encontro Temozon con acento" }
    ...
    # d) imagenes no deben usar rutas VEXO_WEB
    $fotosWeb = ([regex]::Matches($data, "VEXO_WEB")).Count
    $fotosUnsplash = ([regex]::Matches($data, 'images\.unsplash')).Count
    OK "imagenes: $fotosWeb VEXO_WEB | $fotosUnsplash Unsplash"
    if ($fotosWeb -gt 0) { WARN "$fotosWeb rutas VEXO_WEB encontradas en data.js — deben usar images/Desarrollos internas" }
    ...
    $indexHtml = [System.IO.File]::ReadAllText("$BASE\public\index.html", [System.Text.Encoding]::UTF8)
    ...
    foreach ($old in $oldScripts) {
        if ($indexHtml -match "src/data/$old") { ERR "public/index.html aun carga: src/data/$old -- eliminar" }
    }
    if ($indexHtml -match 'src="data\.js"') { OK "public/index.html carga data.js (correcto)" }
    elseif ($indexHtml -match 'src="src/data/data\.js"') { ERR "public/index.html NO debe cargar src/data/data.js" }
    else { ERR "public/index.html NO carga data.js" }
}

if (Test-Path $generatedPath) {
    OK "public/data.generated.js existe"
    $generatedContents = [System.IO.File]::ReadAllText($generatedPath, [System.Text.Encoding]::UTF8)
    $generatedCount = ([regex]::Matches($generatedContents, 'id:\s*\d+,\s*slug:')).Count
    OK "DESARROLLOS en public/data.generated.js: $generatedCount"
    if ($generatedCount -ne 26) { WARN "Se esperaban 26 desarrollos en public/data.generated.js, hay $generatedCount" }
} else {
    WARN "public/data.generated.js no existe"
}

# ================================================================
#  3. IMAGENES FISICAS — .webp en carpetas public/images/Desarrollos
# ================================================================
HDR "3. Imagenes fisicas en public/images/Desarrollos"

$imgTotal = 0; $imgWebp = 0; $imgOtras = 0; $imgSizeMB = 0

if (Test-Path "$BASE\public\images\Desarrollos") {
    Get-ChildItem -Path "$BASE\public\images\Desarrollos" -Recurse -File -ErrorAction SilentlyContinue | ForEach-Object {
        $imgTotal++
    ...
    $carpetas = Get-ChildItem -Path "$BASE\public\images\Desarrollos" -Directory -ErrorAction SilentlyContinue
    ...
    WARN "Carpeta public/images/Desarrollos no encontrada"
}

# ================================================================
#  4. index.html — verificaciones del HTML
# ================================================================
HDR "4. Verificacion public/index.html"
...
if (Test-Path $indexPath) {
    ...
    if ($idx -match 'src="/data\.js"' -or $idx -match 'src="data\.js"') { OK "public/index.html carga data.js (correcto)" }
    else { ERR "public/index.html NO carga data.js" }
...
if (Test-Path $giPath) {
    ...
    foreach ($item in @(".env","*.csv","*.xlsx","Mapa-vexo/","*.log")) {
...
foreach ($item in $gitIgnoreCheck) {
    ...
    if (Test-Path $fp) {
        ...
        $null = git -C $BASE check-ignore -q "$($item.f)" 2>&1
